...
// Ordem preferida de exibição por nome de banco (case-insensitive)
const BANK_ORDER = ['nubank', 'itaú', 'itau', 'inter'];
  ...
  return (
    <View style={{ height: H, backgroundColor: W.plankShadow, overflow: 'hidden', position: 'relative' }}>
      ...
      <View style={{
        ...
        {/* Textos */}
        {/* Título — topo da placa */}
        <Text style={{
          position: 'absolute', left: 0, right: 0, top: 6,
          lineHeight: 32,
          textAlign: 'center',
          fontFamily: 'VT323', fontSize: 34, letterSpacing: 5,
          color: W.gold,
          textShadowColor: W.amber,
          textShadowOffset: { width: 0, height: 0 },
          textShadowRadius: 8,
        }}>A TAVERNA</Text>
        {/* Mês/ano — base da placa */}
        <Text style={{
...
export default function DashboardScreen() {
  ...
  const realmAccounts = [...activeAccounts]
    .sort((a, b) => {
      const ai = BANK_ORDER.indexOf(a.bank.toLowerCase());
      const bi = BANK_ORDER.indexOf(b.bank.toLowerCase());
      return (ai === -1 ? 99 : ai) - (bi === -1 ? 99 : bi);
    })
    .map(account => {
      // balance_cents do servidor já inclui todos os lançamentos sincronizados.
      // Aplicamos aqui apenas os lançamentos criados localmente ainda não sincronizados,
      // detectados pela ausência de correspondência no momento do pull (dirty).
      // Por ora, usamos balance_cents direto — correto após sync.
      const bal = account.balance_cents;
      return { ...account, currentBalanceCents: money(bal) };
    });
              ...
              <React.Fragment key={account.id}>
                <View style={styles.accountRow}>
                  <View style={styles.accountLeft}>
                    <BankIcon bank={account.bank.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')} size={28} />
                    <View>
        ...
        {topCategories.length === 0 ? (
          ...
          <View style={{ gap: 12 }}>
            {topCategories.map((cat, idx) => (
              <View key={`${cat.name}-${idx}`} style={{ gap: 5 }}>
